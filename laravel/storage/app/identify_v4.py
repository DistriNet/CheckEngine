...
logging.basicConfig(filename='/var/www/app/storage/app/identify.log')
logger = logging.getLogger(__name__)


def chrome_to_opera_version(chrome_version, version_table):
    if chrome_version == 'inf':
        return version_str_to_int(chrome_version)
    if int(chrome_version) <= 28:
        return version_str_to_int('15')
    for key, data in version_table["opera"]["versions"].items():
        if data["engine_version"] == "Blink_" + chrome_version:
            return key
    return chrome_to_opera_version(str(int(chrome_version) - 1), version_table)
    ...
    for element, supported in fingerprint.items():
        ...
        if attribute:
            try:
                ...
                logger.error(f"Could not find attribute '{attribute}' for '{element_str}'")
                # raise Exception(f"Could not find attribute '{attribute}' for element '{element}'")
        ...
        for browser in result_table.keys():
            ...
            if 'prefix' in compat_data or 'alternative_name' in compat_data or 'flags' in compat_data or ('version_added' not in compat_data and compat_data != 'mirror'):
                skip_element = True
                break
        if skip_element:
            continue
        ...
        for browser in result_table.keys():
            ...
            if version_added.startswith('≤'):
                ...
                uncertain = True
            if browser_to_check == 'chrome' and browser == 'opera':
                version_added_int = chrome_to_opera_version(version_added, result_table)
            else:
            ...
            if 'version_removed' in compat_data:
                ...
                if not (version_removed is None or version_removed is True or version_removed is False):
                    if browser_to_check == 'chrome' and browser == 'opera':
                        version_removed_int = chrome_to_opera_version(version_removed, result_table)
                    else:
        ...
        if not browser_data["excluded"]:
            for version, value in browser_data["versions"].items():
                ...
                browser_ranking.append({
                    ...
                    'votes_ratio': vote_for / (vote_for + vote_against),
...
def write_csv(data, filename):
    with open(filename, 'w') as csvfile:
        ...
        writer = csv.DictWriter(csvfile, fieldnames=[
            'version', 'votes_for', 'votes_against', 'votes_net', 'votes_total', 'votes_ratio', 'release_date']
        )
        writer.writeheader()
